    ...
    for (var l in mirrorSystem.libraries.entries) {
      if (l.key.toString().contains('/api_model/')) {
        for (var d in l.value.declarations.entries) {
          ...
          // Only inspect classes that are not also mixins
          if (dm is ClassMirror) {
            final className = getSymbolName(dm.simpleName);
            if (!className.endsWith('Mixin')) {
              final superName = getSymbolName(_getTopMostSuperclass(dm));
              ...
              if (!classExceptions.contains(className)) {
                ...
                if (className.endsWith('Collection') || (className == 'AddressBook')) {
                  ...
                  final implementsNames =
                      dm.superinterfaces.map((x) => getSymbolName(x.simpleName));
...
Symbol _getTopMostSuperclass(ClassMirror dm) {
  ClassMirror sc = dm;
  while ((sc.superclass != null) && (getSymbolName(sc.superclass!.simpleName) != 'Object')) {
    sc = sc.superclass!;
  }
  return sc.simpleName;
}
