...
Function GetPubDevPackageLicense {
    ...
    if ($global:KnownGplPubDevPackageNames.Contains($PackageName)) {
        return 'GPL'
    }
...
Function GetGitHubRepoLicense {
    ...
    if (($jsonData -eq $null) -Or ($jsonData.license -eq $null)) {
        return $null;
    }
...
Function Default {
    ...
    try {
        $pubspecYamlFilePath = 'pubspec.yaml'
...
# To speed up verification, maintain a list of known invalid package names
$global:KnownGplPubDevPackageNames = @()
